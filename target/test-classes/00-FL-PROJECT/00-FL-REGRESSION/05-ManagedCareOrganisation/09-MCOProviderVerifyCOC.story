...
Scenario:MCO Provider COC flow to Approve
Meta:
...
When implicit wait '5' seconds
When upload new 'Additional Document (Optional)' document text_file.txt
And implicit wait '5' seconds
...
When implicit wait '5' seconds
Then rewrite stage of credentials to 'ApprovedReviewCOC1' for 'MCO3'

Scenario:MCO Provider COC flow to Deny
Meta:
Given take firstname, lastname and email credentials for type 'MCO3' and 'Approved' and use in session
Given user navigated to landing page
When new user logs to Provider Enrollment portal with username - ${email} and password - Aa123321!
...
Then save coc request ID to deny the request of the provider
When implicit wait '5' seconds
Then rewrite stage of credentials to 'DeniedReviewCOC2' for 'MCO3'