...
Narrative:
As the DYP IU,
I can search CoC based on NPI, FEIN/SSN and Assignee

Scenario:Creating COC as an approved provider
Meta:
...
And implicit wait '5' seconds
And click on CoC tab on header
When search for provider by name '${firstName} ${lastName}' with status 'PENDING APPROVAL' and get specific request id and save it to 'requestId' with delay '10'
When reset filters
And implicit wait '5' seconds
And click on CoC tab on header
And search for enrollment data on IU portal by next parameters:
|requestID   |providerIDorName        |cocType        |feinSSN    |requestStatus   |npi       |assignee  |fromDate|toDate |
|${requestId}|${firstName} ${lastName}|Address Details|456-76-5451|Pending Approval|1225533300|Lewis Wood|${date} |${date}|
