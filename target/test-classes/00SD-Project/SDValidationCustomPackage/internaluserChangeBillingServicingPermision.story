...
Narrative:
As the DYP system,
system should not allow the internal users
to change the individual providers from billing to servicing and vice versa
...
Scenario: Verify that the system is showing pop-up message on changing the selection in Medicaid payment
Meta:
...
When move to 'Identifying Information' section
And implicit wait '5' seconds
When click edit for Medicaid Payment
And implicit wait '5' seconds
And To fill the identifying information section for Enrollment:
|paymentrecived|
|No           |
...
Scenario: Approving the enrollment request
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Created' and use in session
Given user navigated to landing page
When new user logs to Internal User portal with username - internal.user.dyp+cd@gmail.com and password - Internaluser1!
...
Scenario:Verifying that the system is showing pop-up message on changing the selection in Medicaid payment
Meta:
