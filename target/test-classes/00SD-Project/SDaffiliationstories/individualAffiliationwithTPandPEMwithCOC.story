...
Narrative:
As the DYP Individual provider,
I can create CoC to add affiliation with TP and PEM
...
Scenario:Verify that the system is not showing service location in affiliations result for individual to PEM affiliations
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given user navigated to landing page
When new user logs to Provider Enrollment portal with username - ${email} and password - Aa123321!
And implicit wait '5' seconds
...
When click Add Affiliation button in affiliation section
When click search by npi in affiliation section
When select affiliation row by 'PEM' in affiliation section
