...
Narrative:
Individual Enrollment form - As the SD registered Individual provider,
I want to capture my signature of addendum section based on conditions
for individual enrollment form in a format so that it meets the SD need.
...
And implicit wait '5' seconds
When verify the presence of below 'PCP Addendum' agreement parameters:
|npi   |providerName            |
...
And To fill the Program Participation section info:
|licenseCertificateType|issuer      |licenseCertificateNumber|effectiveEndDate2|effectiveStartDate2 |
|Psychology            |South Dakota|54321                   |${expDate}       |${date}             |
When implicit wait '5' seconds
When upload document 'Upload License/Certificate Document*' document text_file.txt in Program Participation section
When implicit wait '10' seconds
When click on 'Add' in Program participation section
...
And implicit wait '5' seconds
When verify the presence of below 'Mental Health Addendum' agreement parameters:
|npi   |providerName            |
|${npi}|${firstName} ${lastName}|
When click on sign button in the provider agreement section
And implicit wait '10' seconds
