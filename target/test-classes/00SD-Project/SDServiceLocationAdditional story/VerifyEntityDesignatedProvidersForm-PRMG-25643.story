...
Narrative:
As a DYP Entity,
if there is a taxonomy that doesn’t allow affiliation
to an individual then the system should display a pop-up message
to add affiliation with a PEM or TP in add affiliation.
...
|${licenseType}        |${licenseIssuedState}|${licenseNumber}        |${expDate}       |${date}             |
When implicit wait '5' seconds
...
When To fill the Service Location info:
|firstname   |lastname   |credentials|npi   |effectiveStart|effectiveEnd|
|${firstName}|${lastName}|CSW        |${npi}|${date}       |${expDate}  |
When implicit wait '3' seconds
Then save designated providers
When implicit wait '2' seconds
When To fill the Service Location info:
|officeLocation|billingServices|familyPlanning |esrdServices  |groupSpeciality|
|Yes           |OP Rehab       |Family Planning|Hospital Based|Vision         |
When implicit wait '2' seconds
When upload document 'Claims Documents' document text_file.txt in Service Location Section
When implicit wait '2' seconds
And To fill the Service Location info:
|priorAutorization|
|Yes              |
...
When click on 'Save' in Service Location section
And implicit wait '5' seconds
When move to 'Health Home Services' section
And implicit wait '5' seconds
Then To fill the Health Home Services Details:
|comprehensiveCareManagement|careCoordination|healthPromotion|comprehensiveTraditionalCare|familySupportServices|
|Comprehensive response     |Care response   |Health response|Traditional response        |Family response      |
Then To fill the Health Home Services Details:
|socialSupportServices|emergencyRoomUtilization|healthHome     |notificationOfEmergencyRoom|primaryCarePractice|
|Social response      |Emergency response      |Health response|Notification response      |Practice response  |
Then To fill the Health Home Services Details:
|applicableResponse|
|Able to submit outcome measures electronically as defined in the data layout.|
When implicit wait '5' seconds
...
Examples:
|index         |providerLegalBusinessName|dbaName|dob       |ssn          |fein      |mailingAddressLine1|mailingAddressZip|mailingAddressCountycode|phone        |taxonomyCategory|primaryTaxonomy          |licenseNumber|licenseType        |licenseIssuedState|primaryServiceLocationAddressLine1|primaryServiceLocationZip|primaryServiceLocationCountyCode|groupClaimSubmit                |secondaryServiceLocationAddressLine1|profitStatus          |reasonCode |phoneno   |managingEmployeeType |employeeStatus         |countryOfBirth|stateofBirth|stateOfBirth |programParticipationTaxonomy    |programParticipation|speciality|
|GroupAdendum1 |SANFORD CLINIC           |Clinic |10/11/1980|456-76-545111|2262618066|2905 5TH Rapid City|33110-4343       |11130                   |(245)357-5678|Interpreter     |171R00000X -  Interpreter|4353454445   |Rural Health Clinic|South Dakota      |2905 5TH Rapid City               |33310-2345               |33140                           |Electronic Data Interchange EDI |19 Wyarno Rd, Sheridan WY           |Non-Profit 501(C)(3)  |3          |3456789098|Manager              |Managing Non Convicted |United States |0           |Alabama      |261QR1300X - Rural Health Clinic|Medicaid/CHIP       |Clinics   |
