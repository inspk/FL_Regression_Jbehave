...
Narrative:
As the DYP IU,
I can search for the CoCs related to service location
with additional parameters such as program participation, specialty, taxonomy & License type.
...
Scenario: Create coc for Program Participation
Meta:
...
Scenario: Approve Coc by 1 approver,2 reviewers to Deny COC
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given user navigated to landing page
When new user logs to Internal User portal with username - wy.internalusers+frank.walker@gmail.com and password - Aa123321!
...
And implicit wait '3' seconds
And search for enrollment data on IU portal by next parameters:
|cocProgramParticipation|cocSpeciality|cocTaxonomy           |cocLicenseType|
|Medicaid/CHIP          |Vision       |152W00000X-Optometrist|DEA           |
And implicit wait '3' seconds
When click to provider data with text '${firstName}' on IU portal
When click on 'Approve' button on decision panel
When select approve reason 'Positive Enrollment History' in Approval popup
