...
I should be able to view effective start date and end date
in service location list and able to view only ended service locations
by selecting the check box in search criteria against each provider.
...
When implicit wait '5' seconds
And To fill the Service Location info:
|locationName  |addressLine1                         |attentionLine|email   |phoneNumber|
|Primary clinic|${primaryServiceLocationAddressLine1}|${firstName} |${email}|${phoneno} |
When implicit wait '5' seconds
...
|Secondary clinic|${primaryServiceLocationAddressLine1}|
When implicit wait '10' seconds
When click on 'Save' in Service Location section
And implicit wait '5' seconds
When To search the Service Location info:
|endedServiceLocation|
|Yes                 |
And implicit wait '5' seconds
Then verify that the system is showing the '${date}' and '${date}' in the Service location result
When implicit wait '5' seconds
...
Scenario: Verify in IU provider data page
Meta:
...
When click to provider data with text '${firstName}' on IU portal
And implicit wait '5' seconds
When click tab with name 'Application' on provider data page
And implicit wait '5' seconds
When move to 'Service Location' section
When implicit wait '5' seconds
When To search the Service Location info:
...
Scenario: Verify in PE provider data page
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given save current date to variable 'date'
Given user navigated to landing page
...
When implicit wait '5' seconds
When click tab with name 'Details' on provider data page
And implicit wait '5' seconds
When move to 'Service Location' section
When implicit wait '5' seconds
When To search the Service Location info:
