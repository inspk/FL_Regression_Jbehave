...
Narrative:
As the DYP Provider,
the Provider can submit request for termination
with the reason code as due to death along with death date
...
Narrative:
As the DYP IU,
I can update the death date for the terminated providers
due to death
...
When save current date plus 1 one day to variable end data 'futureDate'
...
When implicit wait '5' seconds
And To fill the identifying information section for Enrollment:
|title |firstName   |lastName   |gender   |dateOfBirth|countryOfBirth   |stateofBirth   |ssn    |email    |middleName   |taxExempt |businessName   |typeOfBusiness   |dbaName   |
|Mr.   |${firstName}|${lastName}|${gender}|${dob}     |${countryOfBirth}|${stateofBirth}|${ssn} |${email} |${middleName}|Yes       |${businessName}|${typeOfBusiness}|${dbaName}|
When implicit wait '10' seconds
...
When implicit wait '5' seconds
And To fill the Service Location info:
|acceptingNewPatients|ttdTty|county|open24Hours|emergencyServices|patientRecords|personnelRecords|publicPhoneNumber|
|No                  |No    |None  |Yes        |No               |No            |No              |${phoneno}       |
And implicit wait '5' seconds
...
Examples:testdata/SDIndividualFutureApproved.table



Scenario:Approve Enrollment
Meta:
...
Scenario:Login as Provider and Verify that the Death Date is mandatory
Meta:
...
And implicit wait '2' seconds
When select provider 'Due to Death' and 'Death note' in Request Termination popup
And implicit wait '2' seconds
When set death date '' in Request Termination popup
And implicit wait '2' seconds
When select provider 'Death note' in Request Termination popup
And implicit wait '2' seconds
When verify death date message 'Please select death date' in Request Termination popup


Scenario:Login as Provider and Verify that the Death Date is not allowed to enter a future date
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given user navigated to landing page
When new user logs to Provider Enrollment portal with username - ${email} and password - Aa123321!
...
And implicit wait '5' seconds
And save current date to variable 'date'
When save current date plus 1 years to variable 'futureDate'
And close informational pop-up
Then click on the Request Termination Button
Then verifying the Request Termination pop-up is displaying
When set effective date '${date}' in Request Termination popup
...
When select provider 'Due to Death' and 'Death note' in Request Termination popup
And implicit wait '2' seconds
When set death date '${futureDate}' in Request Termination popup
And implicit wait '2' seconds
When select provider 'Death note' in Request Termination popup
And implicit wait '2' seconds
When verify death date message 'Death Date cannot be future date' in Request Termination popup

Scenario:Terminate the Individual Provider with Due to death reason
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given user navigated to landing page
When new user logs to Internal User portal with username - internal.user.dyp+cd@gmail.com and password - Internaluser1!
...
And select provider reactivation reason 'Due to Death' and 'termination note' Provider Reactivation/Termination popup
When implicit wait '5' seconds
When set death date '${date}' in Provider Termination popup
When implicit wait '5' seconds
And click 'Terminate' button reactivate or terminate the provider
When implicit wait '5' seconds
Then verify the Provider Module details 'Death Date' is '${date}'
When implicit wait '10' seconds
When log out from IU portal


Scenario:Login as Provider And request termination for death with upload a document
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given user navigated to landing page
When new user logs to Provider Enrollment portal with username - ${email} and password - Aa123321!
...
When select provider 'Death note' in Request Termination popup
And implicit wait '2' seconds
When upload file text_file.txt in request termination popup
And implicit wait '10' seconds
Then click 'REQUEST TERMINATION' button for the provider
When implicit wait '5' seconds


Scenario:Set new death date from Internaluser end
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given user navigated to landing page
When new user logs to Internal User portal with username - internal.user.dyp+cd@gmail.com and password - Internaluser1!
...
When click to provider data with text '${firstName}' on IU portal
When implicit wait '10' seconds
Then set new death date '${date}' with reason code 'Changed as per the Death certificate'
When implicit wait '10' seconds
When log out from IU portal


Scenario:Verify that the system is showing the Death date in the provider panel of the provider
Meta:
Given take firstname, lastname and email credentials for type 'Individual1' and 'Approved' and use in session
Given user navigated to landing page
And save current date to variable 'date'
...
When click to provider data with text '${firstName}' on IU portal
And implicit wait '5' seconds
Then verify the enrollment request 'Reason Code' is 'Due To Death'
Then verify the enrollment request 'Death Date' is '${date}'
Then verify the enrollment request 'note' is 'Death Note'
When implicit wait '2' seconds
When click on 'Change status' button from the enrollment details page
When set enrollment request status to - Approved
And wait for screening process execution during '5' seconds